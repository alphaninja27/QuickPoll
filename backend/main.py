...
# # Get the deployed frontend URL (no trailing slash, all lowercase)
# FRONTEND_URL = os.getenv("FRONTEND_URL", "https://quickpoll-frontend-mcyo.onrender.com")

# # CORSMiddleware must be registered FIRST, before all routes and websockets!
# app.add_middleware(
#     CORSMiddleware,
#     allow_origins=[
#         FRONTEND_URL,
...
# Allow only your frontend domain, localhost for dev
origins = [
...
@app.get("/polls/")
def get_polls():
    # Simple stub endpoint
    return [{"id": 1, "title": "Test Poll"}]
